...
async fn main() {
    ...
    let mut client_private = Client::new(url, Some(nexdex_PRIVATE_KEY.into())).await.unwrap();
    ...
    let manager = nexdex::ws::WebsocketManager::new(
        nexdex::url::URL::Testnet,
        Some(client_private),
    )
    .await;
    let orders_id = manager
        .subscribe(
            nexdex::ws::Channel::Orders {
                market_symbol: None,
            },
    ...
    let order_request = OrderRequest {
        instruction: nexdex::structs::OrderInstruction::IOC,
        market: symbol,
        price: None,
        side: Side::BUY,
