...
// `GET /system/config`
func GetnexdexConfig() (SystemConfigResponse, error) {
	systemConfigUrl := fmt.Sprintf("%s/system/config", nexdex_HTTP_URL)
...
// Generate nexdex private key from Ethereum private key
func GeneratenexdexAccount(config SystemConfigResponse, ethPrivateKey string) (string, string, string) {
...
func PerformOnboarding(
	...
	// URL
	onboardingUrl := fmt.Sprintf("%s/onboarding", nexdex_HTTP_URL)
	...
	req.Header.Add("nexdex-ETHEREUM-ACCOUNT", ethAddress)
	req.Header.Add("nexdex-STARKNET-ACCOUNT", dexAccountAddress)
	req.Header.Add("nexdex-STARKNET-SIGNATURE", GetSignatureStr(r, s))
...
func GetJwtToken(
	...
	// URL
	authUrl := fmt.Sprintf("%s/auth", nexdex_HTTP_URL)
	...
	req.Header.Add("nexdex-STARKNET-ACCOUNT", dexAccountAddress)
	req.Header.Add("nexdex-STARKNET-SIGNATURE", GetSignatureStr(r, s))
	req.Header.Add("nexdex-TIMESTAMP", timestampStr)
	req.Header.Add("nexdex-SIGNATURE-EXPIRATION", expirationStr)
...
func GetOpenOrders(jwtToken string) []*Order {
	// URL
	ordersUrl := fmt.Sprintf("%s/orders", nexdex_HTTP_URL)
...
func main() {
	// Load nexdex config
	nexdexConfig, _ := GetnexdexConfig()

	// Initialize Ethereum account
	ethPrivateKey, ethAddress := GetEthereumAccount()

	// Generate nexdex account from Ethereum private key
	dexPrivateKey, dexPublicKey, dexAccountAddress := GeneratenexdexAccount(nexdexConfig, ethPrivateKey)
	...
	SubmitOrder(nexdexConfig, dexAccountAddress, dexPrivateKey, jwtToken)
