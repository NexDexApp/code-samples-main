...
def build_auth_message(chainId: int, now: int, expiry: int) -> TypedData:
    message = {
        ...
        "domain": {"name": "nexdex", "chainId": hex(chainId), "version": "1"},
        "primaryType": "Request",
        "types": {
            "StarkNetDomain": [
                {"name": "name", "type": "felt"},
...
def build_stark_key_message(chain_id: int) -> TypedData:
    message = {
        "domain": {"name": "nexdex", "version": "1", "chainId": chain_id},
...
def get_nexdex_account_address(nexdex_config: Dict, nexdex_account_private_key_hex: str) -> str:
    nexdex_key_pair = KeyPair.from_private_key(hex_to_int(nexdex_account_private_key_hex))
    nexdex_account_address = get_acc_contract_address_and_call_data(
        nexdex_config['paraclear_account_proxy_hash'],
        nexdex_config['paraclear_account_hash'],
        hex(nexdex_key_pair.public_key),
    )
    return nexdex_account_address


def generate_nexdex_account(
    nexdex_config: Dict, eth_account_private_key_hex: str
) -> Tuple[str, str]:
    eth_chain_id = int(nexdex_config['l1_chain_id'])
    stark_key_msg = build_stark_key_message(eth_chain_id)
    nexdex_private_key = derive_stark_key_from_eth_key(stark_key_msg, eth_account_private_key_hex)
    nexdex_key_pair = KeyPair.from_private_key(nexdex_private_key)
    nexdex_account_private_key_hex = hex(nexdex_private_key)
    nexdex_account_address = get_acc_contract_address_and_call_data(
        nexdex_config['paraclear_account_proxy_hash'],
        nexdex_config['paraclear_account_hash'],
        hex(nexdex_key_pair.public_key),
    )
    return nexdex_account_address, nexdex_account_private_key_hex
...
def get_account(account_address: str, account_key: str, nexdex_config: dict):
    client = FullNodeClient(node_url=nexdex_config["starknet_fullnode_rpc_url"])
    key_pair = KeyPair.from_private_key(key=hex_to_int(account_key))
    chain = get_chain_id(nexdex_config["starknet_chain_id"])
    account = Account(
        client=client,
...
async def get_jwt_token(
    ...
    chain_id = int_from_bytes(nexdex_config["starknet_chain_id"].encode())
    account = get_account(account_address, private_key, nexdex_config)
