...
* Generates and onboards a new nexdex account based on your Ethereum private key
...
Once you have onboarded, if you would rather use your L2 information directly, be sure to skip over the `Initialize Ethereum` account and `Generate nexdex account` functions in the script. If you simply plug in your L2 private key in place of the `eth_private_key_hex` variable, you will actually be generating a separate account that isn't actually usable.
...
* Withdraws from nexdex (Paraclear Contract) to L1 token bridge contract
...
* Withdraw from nexdex (Paraclear Contract)
...
Script to transfer all USDC tokens from one L2 account to another L2 account on nexdex (Paraclear Contract).

```bash
# pre-req: create venv
OLD_nexdex_ACCOUNT_PRIVATE_KEY=private_key NEW_nexdex_ACCOUNT_PRIVATE_KEY=private_key python transfer_l2_usdc.py
```
...
* Withdraws from nexdex (Paraclear Contract) to L2 Contract (old)
* Triggers transfer of USDC tokens from L2 Contract (old) to L2 Contract (new)
* Deposits to nexdex (Paraclear Contract) from L2 Contract (new)

### Script notes

* Ensure both accounts have previously onboarded via the onboarding example or UI:
  * `OLD_nexdex_ACCOUNT_PRIVATE_KEY` (L2 Private Key of old account)
  * `NEW_nexdex_ACCOUNT_PRIVATE_KEY` (L2 Private Key of new account)
