...
# Transfer USDC from old nexdex account to new nexdex account
...
async def main(old_nexdex_account_private_key_hex, new_nexdex_account_private_key_hex) -> None:
    # Load nexdex config
    nexdex_config = await get_nexdex_config(nexdex_http_url)

    # Get nexdex account addresses
    old_nexdex_account_address = get_nexdex_account_address(
        nexdex_config, old_nexdex_account_private_key_hex
    )
    new_nexdex_account_address = get_nexdex_account_address(
        nexdex_config, new_nexdex_account_private_key_hex
    )
    print(f"OLD nexdex Account Address: {old_nexdex_account_address}")
    print(f"NEW nexdex Account Address: {new_nexdex_account_address}")

    old_account = get_account(
        old_nexdex_account_address, old_nexdex_account_private_key_hex, nexdex_config
    )
    new_account = get_account(
        new_nexdex_account_address, new_nexdex_account_private_key_hex, nexdex_config
    )

    # Remove transfer amount to transfer all available balance
    await paraclear_transfer(nexdex_config, old_account, new_account, transfer_amount=100.0)
...
if __name__ == "__main__":
    ...
    old_nexdex_account_private_key_hex = os.getenv("OLD_nexdex_ACCOUNT_PRIVATE_KEY", "")
    new_nexdex_account_private_key_hex = os.getenv("NEW_nexdex_ACCOUNT_PRIVATE_KEY", "")
    asyncio.run(main(old_nexdex_account_private_key_hex, new_nexdex_account_private_key_hex))
