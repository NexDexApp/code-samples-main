...
async def sign_request(
    nexdex_maker_secret_key: str, method: str, path: str, body: Dict
) -> Tuple[int, bytes]:
    """
    Creates the required signature necessary
    as apart of all RESToverHTTP requests with nexdex.
    """
    _secret_key: bytes = nexdex_maker_secret_key.encode("utf-8")
...
async def create_rest_headers(
    ...
    nexdex RESToverHTTP requests.
    """
    # timestamp, signature = await sign_request(
    #     nexdex_maker_secret_key=nexdex_maker_secret_key,
    #     method=method,
    #     path=path,
    #     body=body
    #     )
    ...
    headers: Dict = {
        # 'nexdex-API-Timestamp': timestamp.decode('utf-8'),
        # 'nexdex-API-Signature': signature.decode('utf-8'),
        "Authorization": f"Bearer {nexdex_jwt}"
    }
...
async def get_open_orders(
    ...
    headers: Dict = await create_rest_headers(
        nexdex_jwt=nexdex_jwt,
        nexdex_maker_secret_key="",
        method=method,
        path=path,
        body="",
    )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(nexdex_http_url + path, headers=headers) as response:
...
async def fetch_trades(nexdex_http_url: str, nexdex_jwt: str, market: str) -> None:
    """
    nexdex RESToverHTTP endpoint.
    [GET] /trades
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            nexdex_http_url + path, headers=headers, params=params
        ) as response:
            status_code: int = response.status
...
def check_token_expiry(status_code: int, response: Dict) -> None:
    """
    Checks the response from the nexdex API
...
async def post_order_payload(nexdex_http_url: str, nexdex_jwt: str, payload: dict) -> dict:
    """
    nexdex RESToverHTTP endpoint.
    [POST] /orders
    """
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                nexdex_http_url + path, headers=headers, json=payload
            ) as response:
                status_code: int = response.status
...
async def delete_order_payload(nexdex_http_url: str, nexdex_jwt: str, order_id: str) -> bool:
    """
    nexdex RESToverHTTP endpoint.
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.delete(nexdex_http_url + path, headers=headers) as response:
                status_code: int = response.status
                response: Dict = await response.json(content_type=None)
                check_token_expiry(status_code=status_code, response=response)
...
async def send_heartbeat_id(websocket: websockets.WebSocketClientProtocol, id: int) -> None:
    """
    Sends a Heartbeat to keep the nexdex WebSocket connection alive.
    """
...
async def send_auth_id(
    websocket: websockets.WebSocketClientProtocol, nexdex_jwt: str, msg_id: str
) -> None:
    """
    Sends an authentication message to the nexdex WebSocket.
    """
...
def starknet_account(config: ApiConfig) -> Account:
    ...
    account = get_account(
        account_address=config.nexdex_account,
        account_key=config.nexdex_account_private_key,
        nexdex_config=config.nexdex_config,
    )
...
async def get_usdc_balance(config: ApiConfig) -> int:
    ...
    usdc_address = config.nexdex_config["bridged_tokens"][0]["l2_token_address"]
...
async def deposit_to_paraclear(config: ApiConfig, amount: int) -> None:
    paraclear_address = config.nexdex_config["paraclear_address"]
    account = starknet_account(config)
    ...
    usdc_address = config.nexdex_config["bridged_tokens"][0]["l2_token_address"]
    usdc_decimals = config.nexdex_config["bridged_tokens"][0]["decimals"]
...
async def get_jwt_token(
    nexdex_config: Dict, nexdex_http_url: str, account_address: str, private_key: str
) -> str:
    logging.info("get_jwt_token")
    token = ""
    chain = int_from_bytes(nexdex_config["starknet_chain_id"].encode())
    account = get_account(
        account_address=account_address, account_key=private_key, nexdex_config=nexdex_config
    )
    now = int(time.time())
    expiry = now + 24 * 60 * 60
    ...
    headers: Dict = {
        ...
        "nexdex-STARKNET-SIGNATURE": flatten_signature(sig),
        "nexdex-TIMESTAMP": str(now),
        "nexdex-SIGNATURE-EXPIRATION": str(expiry),
    }
    path: str = "/auth"
    logging.info(f"get_jwt_token path:{nexdex_http_url + path} headers:{headers}")
    async with aiohttp.ClientSession() as session:
        async with session.post(nexdex_http_url + path, headers=headers) as response:
            status_code: int = response.status
            response: Dict = await response.json()
            if status_code != 200:
...
async def onboarding(
    ...
    headers: Dict = {
        "nexdex-ETHEREUM-ACCOUNT": ethereum_account,
        "nexdex-STARKNET-ACCOUNT": account_address,
        "nexdex-STARKNET-SIGNATURE": flatten_signature(sig),
    }
    path: str = '/onboarding'
    ...
    logging.info(f"onboarding path:{nexdex_http_url + path} headers:{headers}")
    async with aiohttp.ClientSession() as session:
        async with session.post(nexdex_http_url + path, headers=headers, json=body) as response:
            status_code: int = response.status
            if status_code != 200:
...
def get_recovery_phrase(config: ApiConfig) -> str:
    if config.nexdex_environment == "local":
...
def generate_accounts(config: ApiConfig):
    ...
    eth_chain_id = int(config.nexdex_config['l1_chain_id'])
    ...
    config.nexdex_account_private_key = hex(private_key)
    proxy_class_hash = config.nexdex_config['paraclear_account_proxy_hash']
    account_class_hash = config.nexdex_config['paraclear_account_hash']
    ...
    config.nexdex_account = account_address
    print("config.nexdex_account: ", config.nexdex_account)
