...
def stark_key_message(chainId: int):
    message = {
        "domain": {"name": "nexdex", "version": "1", "chainId": chainId},
        "primaryType": "Constant",
        "types": {
            "EIP712Domain": [
...
def get_recovery_phrase_dict(config: dict) -> str:
    if config.get("nexdex_environment") and config.get("nexdex_environment") == "local":
...
def generate_accounts_dict(config: dict) -> dict:
    ...
    eth_chain_id = int(config.get("nexdex_config", {}).get("l1_chain_id"))
    ...
    config["nexdex_account"] = get_acc_contract_address_and_call_data(
        proxy_class_hash,
        account_class_hash,
        hex(key_pair.public_key),
    )
    logging.info(f"{FN} config.nexdex_account: {config['nexdex_account']}")
    return config
