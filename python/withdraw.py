...
async def main(eth_private_key_hex: str) -> None:
    ...
    # Starknet account
    account = get_account(
        nexdex_account_address, nexdex_account_private_key_hex, nexdex_config
    )
    ...
    client, withdraw_tx_hash = await withdraw_from_paraclear(
        eth_account.address, amount, nexdex_config, account
    )
    ...
    await withdraw_from_l1_bridge(eth_account.address, amount, nexdex_config, w3)
