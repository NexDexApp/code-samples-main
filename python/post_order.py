...
from shared.api_client import get_jwt_token, get_nexdex_config, post_order_payload, sign_order
...
async def main(config: ApiConfig) -> None:
    ...
    config.nexdex_account, config.nexdex_account_private_key = generate_nexdex_account(
        config.nexdex_config, eth_account.key.hex()
    )

    # Get a JWT token to interact with private endpoints
    logging.info("Getting JWT...")
    nexdex_jwt = await get_jwt_token(
        config.nexdex_config,
        config.nexdex_http_url,
        config.nexdex_account,
        config.nexdex_account_private_key,
    )

    # POST order
    order = build_order(config, OrderType.Market, OrderSide.Buy, Decimal("0.1"), "ETH-USD-PERP", "mock")
    await post_order_payload(config.nexdex_http_url, nexdex_jwt, order.dump_to_dict())
